...
object PetOrder {
  ...
  object Error {
    case class OrderNotFound(id: PetOrder.Id) extends Error(s"order with id $id not found")
    case class IllegalStatusTransition(from: Status, to: Status)
        extends Error(
          s"can't transition order status from $from to $to"
        )
    case class PetNotFound(id: Pet.Id)        extends Error(s"pet with id $id not found")
